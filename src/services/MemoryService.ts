...
const MemoryService = Object.freeze({
	...
	deletePointsByFilter: async (filterData: Record<string, boolean>) => {
		return await tryRequest(
			apiClient?.api?.memory.deletePointsByFilter(filterData),
			'Memory points deleted successfully',
			'Unable to delete memory points'
		)
	},
})
